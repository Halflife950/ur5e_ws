...
问题来自 `ur_client_library 2.11.0` 中获取 configuration package 的固定 `1s` 超时。`urcl_ws` 不随本仓库提交，它只是一个按需创建的本地 overlay workspace。

如果队友只跑 Gazebo 仿真，或者真机启动没有遇到该 timeout，可以不创建 `urcl_ws`。只有真机启动出现上述报错时，再 clone 官方驱动相关源码，在本地创建：

```text
~/urcl_ws
```

需要复现的核心修改只有一处：把超时时间从：
...
然后重新编译 overlay，让 ROS 2 在运行时优先使用 `~/urcl_ws/install` 中的同名库。这个 workspace 主要服务于真机 timeout workaround，不作为 `ur5e_ws` 主仓库的一部分。

---
## 4. 真机 source 顺序

如果没有使用 `urcl_ws` overlay，真机运行时至少加载：
...
如果为了修复 timeout 创建了 `urcl_ws`，终端环境必须按顺序加载：
...
使用 `urcl_ws` 时，确认当前使用的是 overlay 中的 `ur_client_library`：
...
如果已经创建了 `urcl_ws`，但输出仍然指向 `/opt/ros/humble`，说明 `urcl_ws` 没有 source，或者 source 顺序不对。没有创建 `urcl_ws` 时，指向 `/opt/ros/humble` 是正常的。
...
```bash
source /opt/ros/humble/setup.bash
# 如果创建了 urcl_ws overlay，再 source 下一行
# source ~/urcl_ws/install/setup.bash
source ~/ur5e_ws/install/setup.bash
...
如果队友跑真机时遇到 `Could not get configuration package within timeout`，再按第 III 章说明本地创建 `~/urcl_ws`，并确认：
...
使用 timeout overlay 时，输出应指向 `~/urcl_ws/install/ur_client_library`。如果只跑 Gazebo 仿真，或者真机没有遇到该 timeout，可以不准备 `urcl_ws`。
