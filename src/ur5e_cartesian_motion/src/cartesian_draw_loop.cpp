...
std::vector<geometry_msgs::msg::Pose> makeLineBackAndForth(
    const geometry_msgs::msg::Pose& start_pose)
{
  std::vector<geometry_msgs::msg::Pose> waypoints;

  const int num_points = 50;
  const double length = 0.10; // 10cm
  ...
  {
    geometry_msgs::msg::Pose p = start_pose;
    p.position.x += length - length * static_cast<double>(i) / num_points;
    p.orientation = start_pose.orientation;
    waypoints.push_back(p);
  }
    ...
    const geometry_msgs::msg::Pose& start_pose)
{
  if (mode == "line")
  {
    return makeLineBackAndForth(start_pose);
  }

  if (mode == "square")
  {
    return makeSquare(start_pose);
  }

  if (mode == "circle")
  {
    return makeCircle(start_pose);
  }
...
int main(int argc, char* argv[])
{
  ...
  auto node = rclcpp::Node::make_shared("cartesian_draw_loop");
  ...
  if (mode != "line" && mode != "square" && mode != "circle")
  {
    RCLCPP_ERROR(
        node->get_logger(),
        "Invalid mode. Please choose line, square, or circle.");
  ...
  geometry_msgs::msg::Pose reset_pose = move_group.getCurrentPose().pose;
  geometry_msgs::msg::Quaternion fixed_orientation = reset_pose.orientation;
  ...
  while (rclcpp::ok())
  {
    ...
    std::vector<geometry_msgs::msg::Pose> waypoints = makeWaypoints(mode, start_pose);

    if (waypoints.empty())
    {
      RCLCPP_ERROR(node->get_logger(), "No waypoints generated.");
      break;
    }
    ...
    RCLCPP_INFO(
        node->get_logger(),
        "Cycle %d, Cartesian path fraction: %.3f",
        cycle_count,
        fraction);
    ...
    if (fraction < 0.95)
    {
      RCLCPP_WARN(
          node->get_logger(),
          "Path planning incomplete. Stop loop. Fraction: %.3f",
          fraction);
      break;
    }
    ...
    RCLCPP_INFO(node->get_logger(), "Executing cycle %d...", cycle_count);
  ...
  RCLCPP_INFO(node->get_logger(), "Exit cartesian draw loop.");
